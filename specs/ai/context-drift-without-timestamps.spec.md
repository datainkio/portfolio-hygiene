...
2. Baseline file `/aix/context/drift-baseline.json` with shape:
   ```json
   {
...
- Add `--baseline <hash>` and `--set-baseline` (writes `/aix/context/drift-baseline.json` with resolved hash and optional `--note`).
...
- When `--set-baseline` provided: write `/aix/context/drift-baseline.json` with selected hash and optional note; recalc scores to confirm 0 drift.
...
- `/aix/context/drift-baseline.json` (authoritative baseline; no timestamps).
...
2. Introduce `/aix/context/drift-baseline.json` template (added on first `--set-baseline`).
